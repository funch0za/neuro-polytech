...
def gradient(epochs, train_img, train_labels, learning_rate, weight_hid, weight_out):
    ...
    for i in range(epochs):
        ...
        for j in range(len(train_img)):
            ...
            correct_answers += bool(np.argmax(layer_out) == np.argmax(train_labels[j:j+1]))
            ...
            '''
            layer_hid_delta = layer_out_delta.dot(weight_out.T) * reluderiv(layer_hid) * dropout_mask * 2
...
def handwritten_detection(hidden_size):
    ...
    weight_hid, weight_out, middle_accuracy = gradient(EPOCHS_CNT, train_img, train_labels, LEARNING_RATE, weight_hid, weight_out)
