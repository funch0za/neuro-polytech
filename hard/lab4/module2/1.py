'''
Сделайте пакетный градиентный спуск для распознавания рукописных цифр
'''
...
def gradient(epochs, train_img, train_labels, learning_rate, weight_hid, weight_out, dropout_rate, batch_size):
    middle_accuracy = 0
    for i in range(epochs):
        ...
        for j in range(len(train_img) // batch_size):
            ...
            for k in range(batch_size):
                correct_answers += bool(np.argmax(layer_out[k : k + 1]) == np.argmax(train_labels[batch_start + k : batch_end + k + 1]))
...
def check(test_img, test_labels, weight_hid, weight_out):
    ...
    for j in range(len(test_img)):
        ...
        layer_hid = relu(np.dot(layer_in, weight_hid))
        layer_out = np.dot(layer_hid, weight_out)
        correct_answers += bool(np.argmax(layer_out) == np.argmax(test_labels[j:j + 1]))
...
def handwritten_detection(hidden_size, dropout_rate):
    ...
    weight_hid, weight_out, middle_accuracy = gradient(EPOCHS_CNT, train_img, train_labels, LEARNING_RATE, weight_hid, weight_out, dropout_rate, BATCH_SIZE)
