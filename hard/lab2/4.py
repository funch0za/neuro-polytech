'''
Измените количество эпох обучения (num_epochs). 
Увеличьте или уменьшите это число. 
Как это влияет на ошибку обучения и точность предсказания?
'''
import numpy as np
...
def main():
    ...
    neuro_training(inp, true_prediction, LEARNIN_RATE, 100, LAYER_HIDDEN_SIZE)
    
    # результат стал намного хуже, 10 итераций недостаточно для обучения нейросети
    neuro_training(inp, true_prediction, LEARNIN_RATE, 10, LAYER_HIDDEN_SIZE)

    # результат стал лучше
    neuro_training(inp, true_prediction, LEARNIN_RATE, 500, LAYER_HIDDEN_SIZE)
    
    # результат такой же как при num_epoch = 500, 
    # для улучшения результата нужно менять другие параметры модели
    neuro_training(inp, true_prediction, LEARNIN_RATE, 10 ** 5, LAYER_HIDDEN_SIZE)
