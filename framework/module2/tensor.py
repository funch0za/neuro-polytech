...
class Tensor(object):
    ...
    def __init__(self, data, creators=None, operation_on_creation=None, autograd=False, id=None):
        ...
        if creators is not None:
            for creator in creators:
                creator.children[self.id] = 1 if self.id not in creator.children else creator.children[self.id] + 1
    ...
    def __add__(self, other):
        ...
        return Tensor(self.data + other.data, [self, other], "+", True) if self.autograd and other.autograd else Tensor(self.data + other.data)
